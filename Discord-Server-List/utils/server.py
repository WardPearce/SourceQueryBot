...
class Server(object):
    ...
    async def smart_presence(self, players, max_players, title, loop_index):
        ...
        await self.obj.change_presence(
            status=discord.Status.online,
            activity=discord.Game(
                self.obj.TRANSLATIONS["smart_presence"].format(
                    players,
                    max_players,
                    title)
            )
    ...
    async def format_message(self):
        ...
        for name, values in self.obj.config_cache.items():
            embed = discord.Embed(
                title=self.obj.TRANSLATIONS["title"].format(name),
                colour=discord.Colour(self.obj.CONFIG["bot"]["embed_color"])
            )
            ...
            for server in values["servers"]:
                ...
                if server_info:
                    if not self.obj.CONFIG[
                            "servers"][name]["servers"][ip_port]:
                        server_name = server_info["hostname"][
                            :self.obj.CONFIG["servers"][name]["char_limit"]]
                    else:
                        server_name = self.obj.CONFIG[
                            "servers"][name]["servers"][ip_port]

                    embed.add_field(
                        name=server_name,
                        value=self.obj.TRANSLATIONS["successful"].format(
                            server_info["map"],
                            server_info["players"],
                    ...
                    embed.add_field(
                        name=server_name,
                        value=self.obj.TRANSLATIONS["fail"],
                        inline=False
                    )
            ...
            if values.get("msg"):
                ...
                try:
                    self.obj.config_cache[name]["msg"] = await values[
                    ...
                    try:
                        await self.obj.config.update(
                            values["channel"].id,
                            self.obj.config_cache[name]["msg"].id
                        )
                    except Exception as error:
        ...
        if not self.obj.CONFIG["smart_presence"]["enable"]:
            await self.obj.change_presence(
                status=discord.Status.online,
                activity=discord.Game(
                    self.obj.TRANSLATIONS["total_players"].format(
                        total_players,
                        total_max_players
                    )
                )
